...
class Tree_constructor:
    ...
    def build_tree(self):
        ...
        # self.dask_client.submit(self._build_tree_single_AF, af=af)
        # with Pool(6) as p:
        #    p.map(self._build_tree_single_AF, range(AtomFeatures.get_number_of_features()))
        self.root = DevelopNode()
        self.root.children = [self.roots[af] for af in range(AtomFeatures.get_number_of_features())]
        self.root.update_average()

    def _build_tree_parallel(self, af_name):
        self.df_af_split[af_name] = self.df_af_split[af_name].copy(deep=True)

    def build_tree_parallel(self):
        """Dask parallelization of the tree building process"""
