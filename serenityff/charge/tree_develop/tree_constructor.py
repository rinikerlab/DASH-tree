...
class Tree_constructor:
    ...
    def build_tree(self):
        ...
        for i in range(1, self.num_layers_to_build):
            ...
            self.df_work["connected_atom_max_attention"] = self.df_work.apply(
                lambda x: get_connected_atom_with_max_attention(
                    atom_idx=x["idx_in_mol"],
                    layer=0,
                    mol=self.sdf_suplier[x["mol_index"]],
            ...
            self.df_work["connected_atom_max_attention_idx"] = self.df_work.apply(
                lambda x: get_connected_atom_with_max_attention(
                    atom_idx=x["idx_in_mol"],
                    layer=0,
                    mol=self.sdf_suplier[x["mol_index"]],
            ...
            self.df_work[i] = self.df_work.apply(
                lambda x: self.try_to_add_new_node(x, self.sdf_suplier[x["mol_index"]], i), axis=1
            )
