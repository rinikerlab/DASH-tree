...
class MolMorganDataset:
    ...
    def __init__(
        ...
        self._saved_path = os.path.split(self._data_path)[0] + '/' +  self._filename + '_keys.pkl' #define path for pickle file
        ...
        self._key_dict = defaultdict(lambda: defaultdict(lambda: 0, ),) #stores appearance number for each key. Dicts in a dict for each radius
        self._key_index = defaultdict(lambda: 0, ) #save molecule of first appearance for each key
        self._key_nocharge_index = defaultdict(lambda: 0, )
        self._key_dict_nocharge = defaultdict(lambda: defaultdict(lambda: 0,),)
        self._num_mol = None
        self._num_charged_molecule = None
        self._num_not_charged_molecule = None
        self._weights = []
        self._chargedmols = []
        self.ReadData(UseOldData=UseOldData, FilterForChargedMolecules = FilterForChargedMolecules, ConsiderEquivalentKeysOnce = ConsiderEquivalentKeysOnce) #load data
    ...
    def ReadData(
        ...
        if UseOldData and os.path.isfile(self._saved_path):
            ...
            self.CalcData(FilterForChargedMolecules = FilterForChargedMolecules, ConsiderEquivalentKeysOnce = ConsiderEquivalentKeysOnce)
    ...
    def CalcData(
        ...
        for m in self._mols: #loop over all molecules in MolMorganDataset
            ...
            for val in list(fp): #loop over fingerprints of the molecule
                if not ConsiderEquivalentKeysOnce:
                    self._key_dict[2][val] += len(fp[val]) #counting appearances of different keys (with taking all equivalent atoms into account)
                    ...
                    self._key_dict[2][val] += 1 #counting appearances of different keys (with counting equivalent atoms as one)
                    if fp[val][0][1] < 2: #fingerprints for radius 1
                        self._key_dict[1][val] += 1
                    if fp[val][0][1] < 1: #fingerprints for radius 0
                        self._key_dict[0][val] += 1    
                if self._key_index[val] == 0: #store molecule where each key appears first --> used if a key should be visualized
            ...
            if charged == 0: #calculate keys only for uncharged molecules
                for val in list(fp): #loop over fringerprint key
                    if not ConsiderEquivalentKeysOnce:
                        self._key_dict_nocharge[2][val] += len(fp[val]) #counting appearances of different keys (with taking all equivalent atoms into account)
                        if fp[val][0][1] < 2:  #fingerprints for radius 1
                            self._key_dict_nocharge[1][val] += len(fp[val])
                        if fp[val][0][1] < 1: #fingerprints for radius 0
                            self._key_dict_nocharge[0][val] += len(fp[val])    
                    else: 
                        self._key_dict_nocharge[2][val] += 1 #counting appearances of different keys (with counting equivalent atoms as one)
                        if fp[val][0][1] < 2: #fingerprints for radius 1
                            self._key_dict_nocharge[1][val] += 1
                        if fp[val][0][1] < 1: #fingerprints for radius 0
                            self._key_dict_nocharge[0][val] += 1 
                    if self._key_nocharge_index[val] == 0: #store molecule where each key appears first --> used if a key should be visualized
        ...
        storable = (keydict, keydictnocharge, keyindex, keynochargeindex, self._num_mol, self._num_charged_molecule, self._weights)
        with open(self._saved_path, 'wb') as handle: #write pickle file
            pickle.dump(storable, handle, protocol=pickle.HIGHEST_PROTOCOL) #taken from https://stackoverflow.com/questions/11218477/how-can-i-use-pickle-to-save-a-dict-or-any-other-python-object
        return
    ...
    def weight_distribution(
        ...
        if not IgnoreH:
            ...
            plt.hist(self._weights, bins = range(int(numpy.floor(min(self._weights))), int(numpy.ceil(max(self._weights))) + 1, 1)) #plot weightdistribution as scatterplot
            ...
            plt.hist(weight, bins = range(int(numpy.floor(min(weight))) - int(numpy.floor(min(weight)))%2, int(numpy.ceil(max(weight))) + int(numpy.ceil(max(weight)))%2, 2)) #make bin width of 2 as it produces substructure for uneven weights otherwise. %2 terms to ensure range is dividable by 2
            plt.xlabel('Molecular weight [u]')
            plt.ylabel('Counts')
            plt.title('Weight distribution of dataset ' + self._filename)
            plt.show()
    ...
    def intersection(
        ...
        """        
        set1 = set([Chem.MolToSmiles(m1, isomericSmiles = False) for m1 in self._mols]) #create set of smiles codes for each molecule
        ...
        print('from the ', self._num_mol, ' molecules in ', self._filename, ' ', len(intersect), ' are also in the set ', otherset._filename )
        return intersect #return list of smiles that are present in both sets

    def compare(
        self, 
        otherset, 
        UseChargedMolecules:bool = False
        ) -> None:
        ...
        """        
        print('comparing... ')
        self._sort = defaultdict(lambda: 0,) #stores difference for keys
        self._color = defaultdict(lambda: defaultdict(lambda: 0,),) #store color dependent on number of appearances
        self._nosingletons = defaultdict(lambda: defaultdict(lambda:0,),) #store differents for keys excluding singletons

        col = {} 

        for radius in reversed(range(0,3)): #loop over radius 2, 1 and 0
            hold = defaultdict(lambda: 0,)
            added = defaultdict(lambda: 0,)
            cnew = defaultdict(lambda: 0,)
            cold = defaultdict(lambda: 0,)
            ...
            added = {key: otherkeys.get(key, 0) + ownkeys.get(key, 0) #create dictiionary with all keys in old and new set and add their appereances
                for key in set(otherkeys) | set(ownkeys)}
            metric = {key: (ownkeys.get(key, 0) - otherkeys.get(key, 0))/added.get(key, 0) #create dictionary with the calculated metric to measure the difference in the data set: 1 == only appear in new set; -1 == only in old set
                for key in set(otherkeys) | set(ownkeys)}
            t = {k:v for k, v in sorted(metric.items(), key=lambda item: item[1], reverse = True)} #dict with keys sorted by metric decreasingly
            for i in range(1,5): #we want to distinguish between appearances between 1 to >5
                ...
                shrink(cold,i)
                add = {key: cold.get(key, 0) + cnew.get(key, 0) #recalculate difference without deleted items
                        for key in set(cold) | set(cnew)}
                add_met = {key: (cnew.get(key, 0) - cold.get(key, 0))/add.get(key, 0) 
                        for key in set(cold) | set(cnew)}
            ...
            for (k,v) in t.items():  #finally add all keys that were left over
                if k not in hold.keys():
                        hold[k] = v
                        col[k] = 4
            self._col_string = ['firebrick', 'blue', 'orange', 'green', 'black'] #red: singletons; blue: keys that only appear twice; orange: 3x; green: 4x; black: more than 5
            self._sort[radius] = {k:v for k, v in sorted(hold.items(), key=lambda item: item[1], reverse = True)} #Store sorted dict as entry in another dict where the key is the respective radius
            for k in self._sort[radius].keys():
                self._color[radius][k] = self._col_string[col[k]] #convert number for color to string with color for later plotting
            ...
            for key,value in self._sort[radius].items():
                if added[key] == 1:    #only appear once in one of the data sets
                    ...
                    if added[key] == 2 and self._sort[radius][key] == 0: #appear twice in total but only in one data set, so they are no singleton 
                        continue
    ...
    def plot_compare(self, otherset):
        ...
        #Radius 2
        ax_dict["A"].scatter(list(range(0,len(self._sort[2]))), list(self._sort[2].values()), color = list(self._color[2].values()), s = 0.2) #plot all keys ordered by their difference-value
        ax_dict["A"].hlines(y = 0, xmin = 0, xmax = len(self._sort[2]), color = 'black', linestyle = 'dashed', alpha = .7, linewidth = .75)
        ax_dict["A"].vlines(x= len(self._sort[2])/2.0, ymin = -1, ymax= 1, color = 'black', linestyle = 'dashed', alpha = .7, linewidth = .75)
        ax_dict["A"].set_title(label = 'With singletons')
        ax_dict["A"].set_ylabel(ylabel = 'Fingerpint radius: 2')
        ax_dict["A"].text(x = 0, y = 0.05, s = self._filename, alpha = .5, style = 'italic', fontsize = 7.5)
        ax_dict["A"].text(x = 0, y = -0.2, s = otherset._filename, alpha = .5, style = 'italic', fontsize = 7.5)

        ax_dict["B"].scatter(list(range(0,len(self._nosingletons[2]))), list(self._nosingletons[2].values()), s = .2, color = 'black')#same plot but without singletons
        ax_dict["B"].hlines(y = 0, xmin = 0, xmax = len(self._nosingletons[2]), color = 'black', linestyle = 'dashed', alpha = .7, linewidth = .75)
        ax_dict["B"].vlines(x= len(self._nosingletons[2])/2.0, ymin = -1, ymax= 1, color = 'black', linestyle = 'dashed', alpha = .7, linewidth = .75)
        ax_dict["B"].set_title(label = 'Without singletons')
        ax_dict["B"].tick_params(axis='x', labelsize = 7.5)

        #Radius 1
        ax_dict["C"].scatter(list(range(0,len(self._sort[1]))), list(self._sort[1].values()), color = list(self._color[1].values()), s = 0.2)
        ax_dict["C"].hlines(y = 0, xmin = 0, xmax = len(self._sort[1]), color = 'black', linestyle = 'dashed', alpha = .7, linewidth = .75)
        ax_dict["C"].vlines(x= len(self._sort[1])/2.0, ymin = -1, ymax= 1, color = 'black', linestyle = 'dashed', alpha = .7, linewidth = .75)
        ax_dict["C"].set_ylabel(ylabel = 'Fingerpint radius: 1')

        ax_dict["D"].scatter(list(range(0,len(self._nosingletons[1]))), list(self._nosingletons[1].values()), s = .2, color = 'black')
        ax_dict["D"].hlines(y = 0, xmin = 0, xmax = len(self._nosingletons[1]), color = 'black', linestyle = 'dashed', alpha = .7, linewidth = .75)
        ax_dict["D"].vlines(x= len(self._nosingletons[1])/2.0, ymin = -1, ymax= 1, color = 'black', linestyle = 'dashed', alpha = .7, linewidth = .75)

        #Radius 0
        ax_dict["E"].scatter(list(range(0,len(self._sort[0]))), list(self._sort[0].values()), color = list(self._color[0].values()), s = 0.2)
        ax_dict["E"].hlines(y = 0, xmin = 0, xmax = len(self._sort[0]), color = 'black', linestyle = 'dashed', alpha = .7, linewidth = .75)
        ax_dict["E"].vlines(x= len(self._sort[0])/2.0, ymin = -1, ymax= 1, color = 'black', linestyle = 'dashed', alpha = .7, linewidth = .75)
        ax_dict["E"].set_ylabel(ylabel = 'Fingerpint radius: 0')
        ax_dict["E"].set_xlabel(xlabel = 'Enumerated keys', fontsize = 7.5)

        ax_dict["F"].scatter(list(range(0,len(self._nosingletons[0]))), list(self._nosingletons[0].values()), s = .2, color = 'black')
        ax_dict["F"].hlines(y = 0, xmin = 0, xmax = len(self._nosingletons[0]), color = 'black', linestyle = 'dashed', alpha = .7, linewidth = .75)
        ax_dict["F"].vlines(x= len(self._nosingletons[0])/2.0, ymin = -1, ymax= 1, color = 'black', linestyle = 'dashed', alpha = .7, linewidth = .75)
        ax_dict["F"].set_xlabel(xlabel = 'Enumerated keys', fontsize = 7.5)

        #Add legend for line colors
        fig.legend([lines.Line2D([0],[0], ls='-', c=self._col_string[0]),
    ...
    def missings(
        ...
        if UseChargedMolecules:
            self._onlys = {key:value for (key,value) in otherset._key_dict[radius].items() if key not in self._key_dict[radius]} #keys that only appear in other set
            self._single_onlys = {key:value for (key,value) in self._onlys.items() if otherset._key_dict[radius][key] == 1} #only consider singlets of those
            index = otherset._key_index
        else: 
            self._onlys = {key:value for (key,value) in otherset._key_dict_nocharge[radius].items() if key not in self._key_dict_nocharge[radius]} #keys that only appear in other set
            self._single_onlys = {key:value for (key,value) in self._onlys.items() if otherset._key_dict_nocharge[radius][key] == 1} #only consider singlets of those    
        ...
        self._onlymols = defaultdict(lambda: [],)
        fp = {}
        for key in self._onlys:
            AllChem.GetMorganFingerprint(otherset._mols[index[key]], 2, bitInfo = fp)
            self._onlymols[index[key]].append(fp[key][0][0]) #for each index of molecules store from which atom the missing key is produced
        self._onlymols = {k:v for (k,v) in sorted(list(self._onlymols.items()) ,key=lambda t: len(t[1]), reverse=True)} #sort indexes for which one is responsible for the most amount of missing keys
        ...
        if DrawMolecules:
            self._missingmols = [x for x in self._missingmols if Descriptors.ExactMolWt(x) <= 750] #only use molecules smaller than 750u. If large ones are included all molecules will be scaled accordingly and are hard to see
            if len(self._missingmols) != 0:
                grid_img = Draw.MolsToGridImage(self._missingmols, molsPerRow=5, subImgSize=(500, 500), highlightAtomLists= highlights) #Draw molecules with missing keys to grid image
    ...
    def reduce(
        ...
        news = 0
        identicals = 0
        keydictnew = defaultdict(lambda: 0,)
        ranking = {}
        outputpath = self._folder_path + '/' + NewSetName + '.sdf' #define output path
        print('Newfile is created at path: ', outputpath)
        with Chem.SDWriter(outputpath) as w:
            if otherset == 'none': #If no other set is given. Reduce number of molecules in set to minimum needed to have all fingerprints at least as often as the cutoff.(If enough are there)
                print('Removing redundants...')
                for mol in self._mols: #first step: Only keep molecules that contain at least one key appearing 'cutoff' times or less.
                    AllChem.GetMorganFingerprint(mol, 2, bitInfo = fp)
                    for key in fp:
                        if self._key_dict[2][key] < cutoff: #if any key in molecule appears in total less than cutoff, mark as usefull. They are needed no matter what!
                            usefull = 1
                    if usefull == 1: #write usefull molecules to new file
                        if Chem.MolToSmiles(mol) not in smiles[len(Chem.MolToSmiles(mol))]: #check that its not duplicate
                            if Chem.GetFormalCharge(mol) == 0: #check that its neutral molecule
                                smiles[len(Chem.MolToSmiles(mol))].append(Chem.MolToSmiles(mol)) #store smiles for duplicate detection later     
            ...
            for i, mol in enumerate(leftovers):
                ...
                for key in fp:
                    ...
                    ranking[i] = news
                news = 0
            ranking = {k:v for k, v in sorted(ranking.items(), key=lambda item: item[1], reverse = True)} #sort molecules by amount of desired keys --> this is the greedy approach used to solve problem
            ...
            for index in ranking: #go through ranked molecules
                ...
                for key in fp:
                    if keydictnew[key] < cutoff: #see if it still contributes a new key...
                        usefull = 1
                if usefull == 1: #if so, ...
                    if Chem.MolToSmiles(leftovers[index]) not in smiles[len(Chem.MolToSmiles(leftovers[index]))]: #check for duplicate...
                        if Chem.GetFormalCharge(leftovers[index]) == 0: #check for neutrality...
                            smiles[len(Chem.MolToSmiles(leftovers[index]))].append(Chem.MolToSmiles(leftovers[index])) #store smiles for duplicate detection    
