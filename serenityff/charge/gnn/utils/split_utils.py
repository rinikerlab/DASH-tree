...
def split_data_scaffold(data_list: Sequence[CustomData], n_splits: int, split: int) -> Tuple[Sequence[CustomData]]:
    ...
    scaffold_smiles = [
        Chem.MolToSmiles(
                Chem.Scaffolds.MurckoScaffold.MakeScaffoldGeneric(
                    Chem.Scaffolds.MurckoScaffold.GetScaffoldForMol(
    ...
    groups = np.array([
        np.unique(scaffold_smiles).to_list().index(smile) for smile in scaffold_smiles
    ])
...
def split_data_fragment(data_list, n_splits, split, k, N):
    ...
    scaffold_sample = [] #here we randomly sample moles from all the scaffold. you could also sample moles from all the moles!
    ...
    matches_smiles = [] # here we make a nested list, with every sublist representing a fragment, containing all the molecules that contain the fragment
    ...
    for lst in matches_smiles:
        for i in range(len(lst)-1, -1, -1):
            if lst[i] not in visited_frag:
                ...
                lst.pop(i)
    fragment_full2 = [ele for ele in matches_smiles if ele != []] #This ensures that the list contains all the molecules from the dataset once
    ...
    y_data = graph_df.iloc[:,3]
    train_idx_list, val_idx_list = [], []
    gss = GroupShuffleSplit(n_splits= n_splits, train_size=.8, random_state=42) #TODO play around with these parameters
    for split, (train_idx, val_idx) in enumerate(gss.split(X_data, y_data, groups)):
        train_idx_list.append(train_idx)
        val_idx_list.append(val_idx)
    val_idx_split = val_idx_list[split]
