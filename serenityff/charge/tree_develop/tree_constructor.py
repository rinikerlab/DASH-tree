...
from serenityff.charge.tree.tree_utils import get_possible_connected_atom, get_connected_atom_with_max_attention, get_possible_connected_new_atom, get_connected_neighbor, create_new_node_from_develop_node


class Tree_constructor():
    def __init__(self, df_path:str, sdf_suplier:str, nrows:int=1000, attention_percentage:float=0.99, data_split:float=0.2, seed:int=42, num_layers_to_build=24):
        ...
        self.original_df = pd.read_csv(df_path, usecols=["atomtype", "smiles", "mol_index", "idx_in_mol", "node_attentions", "prediction", "truth"], nrows=nrows)

        #split the dataset into build and test
        random.seed(seed)
        test_set = random.sample(self.original_df.mol_index.unique().tolist(), int(len(self.original_df.mol_index.unique())*data_split))
        ...
        self.df["node_attentions"] = self.df["node_attentions"].apply(lambda x: (np.array(x)/sum(x)).tolist()) # normalize node attentions
    ...
    def try_to_add_new_node(self, line, mol, layer):
        try:
            ...
            for child in current_node.children:
                ...
                possible_new_atoms = [(i, atom_features(mol, i, connectedTo=get_connected_neighbor(atom_idx=i, mol=mol, connected_atoms=line["connected_atoms"]))) for i in possible_new_atoms_idx]
            ...
            if matching_child is not None:
                ...
                connectedTo = get_connected_neighbor(atom_idx=current_atom_idx, mol=mol, connected_atoms=line["connected_atoms"])
                new_atom_feature = atom_features(mol, current_atom_idx, connectedTo=connectedTo)
                new_node = develop_node(atom=new_atom_feature, level=layer+1, data=line["truth"], attention_data=line["connected_atom_max_attention"], parent_attention=current_node.parent_attention + np.max(current_node.attention_data))
    ...
    def create_tree_level_0(self):
        ...
        self.df.apply(lambda x: self.root.add_node([develop_node(atom=x[0], level=1, data=x["truth"], attention_data=x["connected_attentions"][0])]), axis=1)
    ...
    def _work_through_df_and_build_tree_step(self, df_local, layer, attention_percentage):
        ...
        df_work["connected_atom_max_attention"] = df_work.apply(lambda x: get_connected_atom_with_max_attention(atom_idx=x["idx_in_mol"], layer=0, mol=self.sdf_suplier[x["mol_index"]], node_attentions=x["node_attentions"])[1], axis=1)
        df_work["connected_atom_max_attention_idx"] = df_work.apply(lambda x: get_connected_atom_with_max_attention(atom_idx=x["idx_in_mol"], layer=0, mol=self.sdf_suplier[x["mol_index"]], node_attentions=x["node_attentions"])[0], axis=1)
        df_work.sort_values(by="connected_atom_max_attention", ascending=False, inplace=True)

        df_work[layer] = df_work.apply(lambda x: self.try_to_add_new_node(x, self.sdf_suplier[x["mol_index"]], layer), axis=1)
        return df_work
    ...
    def build_tree(self):
        ...
        for i in range(1, self.num_layers_to_build):
            ...
            self.df_work["connected_atom_max_attention"] = self.df_work.apply(lambda x: get_connected_atom_with_max_attention(atom_idx=x["idx_in_mol"], layer=0, mol=self.sdf_suplier[x["mol_index"]], node_attentions=x["node_attentions"])[1], axis=1)
            self.df_work["connected_atom_max_attention_idx"] = self.df_work.apply(lambda x: get_connected_atom_with_max_attention(atom_idx=x["idx_in_mol"], layer=0, mol=self.sdf_suplier[x["mol_index"]], node_attentions=x["node_attentions"])[0], axis=1)
            self.df_work.sort_values(by="connected_atom_max_attention", ascending=False, inplace=True)

            self.df_work[i] = self.df_work.apply(lambda x: self.try_to_add_new_node(x, self.sdf_suplier[x["mol_index"]], i), axis=1)
