...
#trainer = Trainer(device = "cuda")
#print(trainer.device)
trainer.model = ChargeCorrectedNodeWiseAttentiveFP().to('cuda')
print(trainer.device)
...
pd.DataFrame(eval_data_to_save, columns=["smiles", "sdf_idx"]).to_csv("training/GNN_lr_%.8f_batch_%i_seed_%i_index_new_set.csv" % (lr,batch,seed), index=False)
